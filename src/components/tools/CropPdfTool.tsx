...
export default function CropPdfTool() {
  ...
  return (
    <div className="max-w-xl mx-auto px-4 py-10">
      <h1 className="text-2xl font-bold text-gray-900 mb-1">Crop PDF</h1>
      <p className="text-gray-500 mb-8">Crop PDF page margins to your specified dimensions.</p>
      ...
      {!file ? (
        ...
        <div className="space-y-5">
          <div className="p-4 bg-gray-50 rounded-xl border border-gray-200">
            ...
            <p className="text-sm text-gray-500">{formatBytes(file.size)} · {pageCount} pages · {pageSize.width} × {pageSize.height} pt</p>
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-3">Margins to crop (in points/pt)</label>
            ...
            <p className="text-xs text-gray-400 mt-2">72pt = 1 inch. Standard A4 page size = 595 × 842 pt.</p>
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Apply to</label>
            <div className="flex gap-2">
              {[{ v: "all", l: "All Pages" }, { v: "odd", l: "Odd only" }, { v: "even", l: "Even only" }].map((o) => (
          ...
          <button onClick={() => { setFile(null); setResultUrl(null); }} className="w-full py-2 text-sm text-gray-500 hover:text-gray-700">Change file</button>
        </div>
      )}
    </div>
  );
