  ...
  async function process() {
    ...
    try {
      ...
      if (fileType === "docx") {
        ...
        const median = sorted.length ? sorted[Math.floor(sorted.length / 2)] : 12;
        ...
        for (let p = 0; p < pages.length; p++) {
          ...
          for (let k = 0; k < items.length; k++) {
            ...
            if (prevY !== null && Math.abs(y - prevY) > (lineHeight || median) * 0.5) {
              if (lineText.trim()) lines.push({ text: lineText.trim(), height: lineHeight });
              lineText = ""; lineHeight = 0;
            }
          ...
          const pageLines = lines.map(({ text, height }) => {
            if (height >= median * 1.8 && text.length < 120) return `# ${text}`;
            if (height >= median * 1.3 && text.length < 120) return `## ${text}`;
        ...
        if (!parts.length) throw new Error("No extractable text found. This may be a scanned PDF — use the OCR tool instead.");
        md = parts.join("\n\n---\n\n");
      }
