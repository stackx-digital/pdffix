  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-10">
      ...
      <label className="flex flex-col items-center justify-center border-2 border-dashed border-gray-300 rounded-xl p-10 cursor-pointer hover:border-red-400 hover:bg-red-50 transition-colors mb-6">
        ...
        <span className="text-sm text-gray-400 mt-1">JPG, PNG — you can select multiple at once</span>
      ...
      {images.length > 0 && (
        <div>
          ...
          <p className="text-sm text-gray-500 mb-4">{images.length} image(s) · {formatBytes(images.reduce((s, img) => s + img.file.size, 0))}</p>
          ...
          <div className="mb-5">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                ...
                <div>
                  <p className="text-sm font-semibold">Merge into 1 PDF</p>
                  <p className="text-xs text-gray-400 mt-0.5">All images in one file</p>
                </div>
              </button>
              <button
              ...
              >
                ...
                <div>
                  <p className="text-sm font-semibold">Separate PDFs</p>
                  <p className="text-xs text-gray-400 mt-0.5">Each image becomes 1 PDF</p>
                </div>
              </button>
            </div>
          </div>
          ...
          {resultUrl && (
            <a href={resultUrl} download="images.pdf" className="flex items-center justify-center gap-2 w-full py-3 bg-green-600 text-white rounded-xl font-medium hover:bg-green-700 mb-3">
              <Download className="w-5 h-5" /> Download PDF ({images.length} page(s))
            </a>
          )}
          ...
          {!resultUrl && resultUrls.length === 0 && (
            ...
            >
              {processing
                ? "Processing..."
                : outputMode === "merge"
                  ? `Merge ${images.length} image(s) into PDF`
                  : `Convert ${images.length} image(s) to separate PDFs`}
            </button>
          )}
