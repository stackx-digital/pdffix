...
export default function PdfFormsTool() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-10">
      <h1 className="text-2xl font-bold text-gray-900 mb-1">Fill PDF Form</h1>
      <p className="text-gray-500 mb-8">Fill in interactive form fields in your PDF directly.</p>
      ...
      {!file ? (
        <label className="flex flex-col items-center justify-center border-2 border-dashed border-gray-300 rounded-xl p-16 cursor-pointer hover:border-red-400 hover:bg-red-50 transition-colors">
          ...
          <span className="text-sm text-gray-400 mt-1">PDF containing interactive form fields</span>
        ...
        <div className="space-y-5">
          <div className="p-4 bg-gray-50 rounded-xl border border-gray-200 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500">{formatBytes(file.size)} · {fields.length} form fields</p>
            </div>
            <button onClick={() => { setFile(null); setFields([]); setNoForms(false); setResultUrl(null); }} className="text-sm text-gray-400 hover:text-gray-600">Change</button>
          </div>

          {noForms ? (
            <div className="p-6 bg-amber-50 border border-amber-200 rounded-xl text-center">
              <FileInput className="w-8 h-8 text-amber-500 mx-auto mb-2" />
              <p className="font-medium text-amber-800">This PDF has no interactive form fields</p>
              <p className="text-sm text-amber-600 mt-1">Use the Edit PDF tool to add text manually.</p>
            </div>
          ) : (
            <>
                  ...
                  <div key={field.name} className="p-4 bg-white border border-gray-200 rounded-xl">
                    ...
                    {field.type === "checkbox" && (
                      <label className="flex items-center gap-2 cursor-pointer">
                        ...
                        <span className="text-sm text-gray-700">{field.checked ? "Checked" : "Unchecked"}</span>
                      </label>
                    )}
              ...
              {resultUrl ? (
                <a href={resultUrl} download={`filled_${file.name}`} className="flex items-center justify-center gap-2 w-full py-3 bg-green-600 text-white rounded-xl font-medium hover:bg-green-700">
                  <Download className="w-5 h-5" /> Download Filled PDF Form
                </a>
              ) : (
